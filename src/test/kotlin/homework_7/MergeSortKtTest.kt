...
internal class MergeSortKtTest {
    ...
    @ParameterizedTest(name = "asyncMergeSort test {index}, {1}")
    fun mergeSortMTTest(expectedList: List<Int>, notSortedArray: IntArray) {
        for (numberOfCoroutines in 1..10) {
            val actualArray = IntArray(notSortedArray.size) { 0 }
            runBlocking {
                notSortedArray.asyncMergeSort(
                    resultArray = actualArray,
            ...
            try {
                ...
                throw AssertionError(
                    "expected: $expectedList\nactual: ${actualArray.toList()}\n" +
                            "number of coroutines = $numberOfCoroutines"
                )
            }
        }
    }
